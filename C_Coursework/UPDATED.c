...
void endProgram() {
    ...
    clear();
    setColour(black);
    drawString("Found all markers successfully", ((GRID_SIZE_X - 1) * SQUARE_SIZE) / 2, ((GRID_SIZE_Y + 2) * SQUARE_SIZE) / 2);
}
...
int canMoveForward(struct Grid *g, struct Robot *r) {
    ...
    if (xToCheck < 0 || xToCheck >= GRID_SIZE_X || yToCheck < 0 || yToCheck >= GRID_SIZE_Y) {
        return 0;
    } else if (g->grid[xToCheck][yToCheck] == 1) {
        return 0;
    } else {
        return 1;
    }
...
void moveUntilPossible(struct Grid *g, struct Robot *r, struct MovementStack *movements) {
...
void turnRightAndMove(struct Grid *g, struct Robot *r, struct MovementStack *movements) {
    ...
    moveUntilPossible(g, r, movements);
}
...
void moveOneStep(struct Grid *g, struct Robot *r, struct MovementStack *movements) {
...
void findMarker(struct Grid *g, struct Robot *r, struct MovementStack *movements) {
    if (!atMarker(g, r)) {
        ...
        while (!atMarker(g, r)) {
            if (canMoveForward(g, r)) {
                ...
                pushMovement(movements, 1, 'R');
                flag = 0;
            } else if ((homeX == 0 && homeY == 0) ||(homeX == 9 && homeY == 9) ) {
                turnLeft(r);
    ...
    gotMarkerAnimation(g, r);
}

void returnHome(struct Grid *g, struct Robot *r, struct MovementStack *movements) {
    turnLeft(r);
    turnLeft(r); // turn 180
    while (movements->top >= 0) {
